...
const allowedCreators = new Set(["super-admin", "owner"]);
...
export async function GET() {
  ...
  const users = await prisma.user.findMany({ where: session.slug === "super-admin" ? {} : { companyId }, include: { role: true, company: true }, orderBy: { createdAt: "desc" }, take: 100 });
  const roles = await prisma.role.findMany({ where: session.slug === "super-admin" ? {} : { companyId }, orderBy: { name: "asc" } });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const user = await prisma.user.create({ data: { companyId, roleId: body.roleId || undefined, name: body.name, email: body.email, passwordHash, isSuperAdmin: body.isSuperAdmin === true && session.slug === "super-admin" }, include: { role: true, company: true } });
    await prisma.activityLog.create({ data: { companyId, action: "Created user", entity: "User", entityId: user.id, metadata: { email: user.email, role: user.role?.slug } } });
